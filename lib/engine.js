...
// A model may be auto-selected only when it is BOTH genuinely image-capable
// per the catalog AND on the native-vision whitelist. Catalog-only image
// capability is untrusted: deployments advertise text-only models as
// image-capable via modelOverrides (so the chat admits pasted images), and
// such models reject image blocks upstream.
function isTrustedVisionModel(model) {
  return NATIVE_VISION_MODELS.has(model && model.id)
...
// AUTO-SELECTION: find the first whitelisted, catalog-image-capable model,
// preferring the caller's provider, then MODEL_PRIORITY order. Providers
// whose only "image-capable" models come from modelOverrides advertisements
// are skipped — only trusted native vision models are ever auto-picked.
export async function resolveVisionRoute(ctx, preferredProvider) {
  for (const provider of registeredProviders(ctx, preferredProvider)) {
    try {
      ...
      const trusted = models.filter(isTrustedVisionModel)
