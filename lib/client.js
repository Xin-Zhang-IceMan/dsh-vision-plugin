 ...
 *  - shows the CURRENT final version;
 ...
 *  - persists the choice in localStorage and re-applies it after a dsh
 *    restart (the host-side route is process-local);
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function createVisionSection(ctx) {
      const slots = ctx.get('slots')
      if (slots === undefined) return
      const locale = ctx.get('locale')
      const ROUTE_SEP = '\u0000'
      const NS = 'vision'
      const STORAGE_KEY = 'dsh-vision-plugin.route'
      ...
      const dict = {
        zh: {
          ...
          nativeTag: '原生视觉',
          allModelsHint: '列表展示所有目录声明支持图片输入的模型（跨全部已配置路由）；标记「原生视觉」的是已验证模型。选择其他模型时，若上游拒绝图片，插件会自动回退到原生视觉模型。',
        ...
        en: {
          ...
          allModelsHint: 'The list shows every catalog image-capable model across all configured routes; "native vision" marks the verified ones. If a chosen model rejects image input upstream, the plugin automatically falls back to a native vision model.',
      ...
      const t = locale === undefined ? (key) => key : locale.bind(NS)
      const storeRoute = (value) => {
        try { localStorage.setItem(STORAGE_KEY, JSON.stringify(value)) } catch (err) { /* storage unavailable; keep session-only */ }
      }
      ...
      if (locale !== undefined) {
        ctx.effect(() => locale.register(NS, dict), 'vision: dictionaries')
      }

      // Re-apply the saved route after a dsh restart (the host-side route is
      // process-local). No-op when nothing was saved or nothing changed.
      ctx.effect(() => {
        getState().then((s) => {
          const stored = loadStoredRoute()
          if (stored === undefined) return
          const current = (s && s.configuredRoute)
            ? { provider: s.configuredRoute.provider, model: s.configuredRoute.model }
            : null
          const storedRoute = stored && stored.model ? { provider: stored.provider, model: stored.model } : null
          if (JSON.stringify(storedRoute) === JSON.stringify(current)) return
          setModel(storedRoute ? { provider: storedRoute.provider, model: storedRoute.model } : { model: null })
            .catch(() => { /* saved model no longer available; host stays as-is */ })
        }).catch(() => {})
        return () => {}
      }, 'vision: restore route')

      slots.inject('settings.section', () => slots.register(
        { name: 'settings.section', id: NS, order: 25, label: () => t('settingsTitle') },
        () => {
          ...
          const save = () => {
            ...
            setModel(args).then((s) => {
              setState(s)
              setSaving(false)
              setSaved(true)
              storeRoute(idx === -1 ? null : { provider: selected.slice(0, idx), model: selected.slice(idx + ROUTE_SEP.length) })
            }).catch((e) => {
          ...
          // Every catalog image-capable model on every configured provider —
          // whitelisted native vision models and any the user declared
          // image-capable via modelOverrides alike. Auto-selection stays
          // whitelist-only; an explicit choice is honored with fallback.
          ...
          return React.createElement('div', { className: 'dsh-vision-card' },
            ...
            React.createElement('div', { className: 'dsh-vision-field' },
              ...
              React.createElement('select', {
                ...
                visionModels.map((m) =>
                  ...
                  }, m.id + ' · ' + m.provider + (m.native ? ' · ' + t('nativeTag') : '')))),
            ...
            React.createElement('div', { className: 'dsh-vision-hint' },
              ...
              React.createElement('div', null, t('allModelsHint')),
              React.createElement('div', null, t('hint')),
              React.createElement('div', null, t('persistHint'))))
        },
      ))
    }
