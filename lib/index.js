 ...
 *    registered through ctx.inject(['webServer'], …) so the row stays
 *    active in stacks without a web server (headless, dsh rc.8's boot
 *    audit fails any loader entry left pending on a missing service);
 ...
 * and the tool registry. `webServer` is optional (see apply) — hard-requiring
 * it here would leave the row pending and fail rc.8's boot audit in stacks
 * without a web server (headless). */
export const inject = ['fs', 'llm', 'attachments', 'tools']
...
export function apply(ctx) {
  ...
  // Register the settings API only where a web server exists (the web-app
  // stack). The inject fiber waits for the service, so the row itself is
  // active immediately — in headless stacks the callback simply never runs.
  ctx.inject(['webServer'], (scope) => {
    ...
    scope.effect(() => scope.webServer.register({
      ...
      handler: async (_req, res) => {
        try {
          sendJson(res, 200, await catalogState(scope))
        } catch (err) {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (model === null) {
            ...
            const provider = await findProviderForModel(scope, model, preferred)
            if (provider === null) {
  ...
  console.log('vision plugin v' + VERSION + ' active — tool + auto-translation ready (host-composition row, loads at boot; settings API follows when webServer exists)')
}
